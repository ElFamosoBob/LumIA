...
**Si jamais il y a quelconque problème, vous pouvez me demander**
...
|---|---|---|
| [`main.js`](../main.js) | la séquence de démarrage, et rien d'autre | tout |
...
Convention :   **les dossiers de code sont en PascalCase**
               **les autres en minuscules**

               
### `Config/` et `Utils/Constant.js` :

Deux fichiers portent des constantes.
...
La caméra est une ressource unique et partagée. Et une énigme laissée dans 
un autre onglet est gelée, pas active en arrière-plan.

Rien ne tourne tant que `isTransitioning` est vrai : c'est ce qui empêche une résolution et ses
...
4. animation de réussite ("énigme réussie"), puis les énigmes débloquées, puis l'objet physique — le tout dans la même
...
Une énigme n'écrit pas `isResolved` elle-même.
...
- **Colors** et le **chatbot** ne font pas partie de la chaîne : ils sont ouverts par des codes tapés
  dans le terminal, que l'équipe trouve dans la salle.
- **L'accusation** exige *deux* conditions : LSF résolue **et** le chatbot ayant réduit ses suspects à
  ...
  devient vraie.
- **L'énigme finale** est le dernier maillon. La résoudre termine la partie.
...
| **Le panneau** | le DOM : ce que voit le joueur. Aucune règle | [`PanelAruco.js`](../UI/Panels/PanelAruco.js) |
...
`Enigma` et n'entre jamais dans le pool actif — il attend des saisies, la boucle n'a donc rien à faire
de lui. Il n'a pas non plus de panneau de victoire : il n'est jamais « résolu ». Il signale ce qu'il a
trouvé par un callback `onCulpritFound` (el famoso flag), et ne sait rien du moteur.

---

## 7. Sauvegarde et restauration

Un rechargement en pleine partie ne doit pas coûter sa progression à l'équipe.
...
parte de zéro. Pendant l'installation, `resetProgression()` dans la console du navigateur fait de même
à la demande.
...
**Les âges se comptent en images, pas en secondes.** Le recognizer Aruco mémorise coins et
homographies pendant un certain nombre d'*images analysées*. À 10 images par seconde, 200 images font
20 secondes. Modifier `fpsTarget` modifie ces tolérances (et augmente le lag surtout dans lsf).
...
`UIManager`, au risque de créer un import circulaire.
...
Deux écritures possibles, et le choix dépend de **qui construit l'objet, et quand** :
...
Il est donc branché après coup. Il lit un code et demande qu'on débloque quelque chose, sans savoir
qui s'en charge. `PanelColors.connectCalibration` et `PanelAruco.connectVerifyButton` suivent le même
